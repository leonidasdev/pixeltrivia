...
export const POST = withErrorHandling(async (request: NextRequest) => {
  ...
  // Parse request body — silently default to empty object for backward
  // compatibility with clients that send no body (playerName is still required
  // and will fail validation below).
  let body: Record<string, unknown> = {}
  try {
    body = await request.json()
  } catch {
    // Empty or malformed body defaults to empty object
  }
