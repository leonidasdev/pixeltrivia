...
export function SectionHeader({
  ...
  return (
    <div className={`mb-4 ${className}`}>
      <h2
        className={`${styles.title} font-bold text-cyan-300 pixel-text-shadow flex items-center gap-2`}
      >
        {icon && <span className={styles.icon}>{icon}</span>}
        {title}
      </h2>
      {description && <p className={`${styles.description} text-gray-400 mt-1`}>{description}</p>}
    </div>
  )
}
